    ...
    public class CustomerController : Controller
    {
        ...
        public ActionResult login(FormCollection fc)
        {
            ...
            var pass = user_account.FirstOrDefault()?.password;
                ...
                {
                    ViewBag.error = "Tên người dùng không chính xác";
                }
                else
                {
        ...
        public ActionResult register(user muser, FormCollection fc)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (Luser.Count() > 0)
                {
                    ViewBag.error = "Tên người dùng có sẵn";
                    return View("loginEndRegister");
                }
                else
                {
                    ...
                    db.users.Add(muser);
                    db.SaveChanges();
                    Message.set_flash("Đăng ký tài khoản thành công, Đăng nhập tại đây", "thành công");
                    return Redirect("~/dang-nhap");
                }

            }
            Message.set_flash("Đăng ký tài khoản không thành công", "dangger");
            return View("register");
        }
        ...
        public ActionResult Myaccount(user user,FormCollection fc)
        {
            ...
            if(pswO != null)
            {
                ...
                {
                    ViewBag.success = "Mật khẩu không đúng.";
                    return View("Myaccount", user);
                }
                else
                {
        ...
        public ActionResult canelOrder(int OrderId)
        {
            ...
            foreach (var item in orderDetail)
            {
                ...
                if (TongSoNgay >= 7)
                {
                    ...
                    if (item == null)
                    {
                        Message.set_flash("Không thể hủy vé", "dangger");
                        return Redirect("~/tai-khoan");
                    }
                ...
                {
                    Message.set_flash("Vé không thể hủy 7 ngày trước khi bay", "dangger");
                    return Redirect("~/tai-khoan");
                }
