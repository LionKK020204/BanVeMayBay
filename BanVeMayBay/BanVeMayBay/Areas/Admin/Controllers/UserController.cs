    ...
    public class UserController : BaseController
    {
        ...
        public ActionResult Create(user muser, FormCollection data)
        {
            if (ModelState.IsValid)
            {
                ...
                if (password1!=password2) {ViewBag.error = "Mật khẩu không hợp lệ"; }
                if (Luser.Count()>0) { ViewBag.error1 = "Tên này đã có người dùng"; }
                else
                {
                    ...
                    db.users.Add(muser);
                    db.SaveChanges();
                    Message.set_flash("Tạo người dùng thành công", "success");
        ...
        public ActionResult Deltrash(int id)
        {
            ...
            {
                Message.set_flash("Không xóa quản trị viên", "danger");
                return RedirectToAction("Index");
            }
            muser.status = 0;
            muser.updated_at = DateTime.Now;
        ...
        public ActionResult deleteTrash(int id)
        {
            ...
            db.SaveChanges();
            Message.set_flash("Đã xóa vĩnh viễn 1 người dùng", "success");
            return RedirectToAction("trash");
        }
